...
export default function App() {
  const { user, openProfile, openSettings } = useAccount();
  ...
  const busy = actionBusy || scopeBusy;
  const [error, setError] = useState('');
  const [toast, setToast] = useState('');
  const [history, setHistory] = useState<RunEntry[]>([]);
  const [scopedDataset, setScopedDataset] = useState<Dataset | null>(null);
  const [quantities, setQuantities] = useState<Record<string, number>>({});
  const [note, setNote] = useState('Проверены сроки поставок и рекомендованные количества.');
  const [confirmed, setConfirmed] = useState(false);
  const uploadRef = useRef<HTMLInputElement>(null);
  const run =
    storedRun?.dataset_id === dataset?.id &&
    (storedRun?.options.warehouse || '') === warehouse &&
    (storedRun?.options.category_id || '') === category
      ? storedRun
      : null;
  const scopeView = ['dashboard', 'orders', 'quality'].includes(page) ? 'run' : page;
  const scopeLabel = `${warehouse || 'Все склады'} · ${dataset?.categories.find((c) => c.id === category)?.name || 'Все категории'}`;

  const changeScope = (nextWarehouse: string, nextCategory: string) => {
    setWarehouse(nextWarehouse);
    setCategory(nextCategory);
    setDetail(null);
    setScopedDataset(null);
    setHistory([]);
    setSupplier('');
    setRisk('all');
    setSearch('');
  ...
  useEffect(() => {
    ...
    let active = true;
    setBusy('Загружаем данные…');
    setError('');
    void (async () => {
      try {
        const data = await api<Dataset>('/dataset');
        const runs = await api<RunEntry[]>(`/runs?dataset_id=${encodeURIComponent(data.id)}`);
        const latest = runs.length ? await api<Run>(`/runs/${runs[0].id}`) : null;
        if (!active) return;
        if (latest) acceptRun(latest);
        else setWarehouse(data.warehouses.includes('Алматы') ? 'Алматы' : data.warehouses[0] || '');
        setDataset(data);
      } catch (e) {
  ...
  useEffect(() => {
    if (!dataset || scopeView === 'partners' || (scopeView === 'run' && run)) return;
    let active = true;
    const query = new URLSearchParams({ warehouse, category_id: category });
    setScopeBusy(scopeView === 'run' ? 'Обновляем прогноз для выбранных условий…' : 'Обновляем данные…');
    setError('');
    if (scopeView === 'data') setScopedDataset(null);
    if (scopeView === 'history') setHistory([]);
    void (async () => {
      try {
        if (scopeView === 'data') {
          const result = await api<Dataset>(`/dataset?${query}`);
          if (active) setScopedDataset(result);
        } else if (scopeView === 'history') {
          query.set('dataset_id', dataset.id);
          const result = await api<RunEntry[]>(`/runs?${query}`);
          ...
          if (active) acceptRun(result);
        }
      } catch (e) {
        if (active) setError(e instanceof Error ? e.message : 'Не удалось обновить данные');
      } finally {
        if (active) setScopeBusy('');
      }
    })();
    return () => {
      active = false;
    ...
    // Scenario settings remain explicit; category and warehouse changes refresh automatically.
  }, [scopeView, warehouse, category, dataset?.id]);
  useEffect(() => {
    if (!toast) return;
    ...
    <div className="app-shell">
      <aside className="sidebar">
        <a
          className="brand"
          href="#"
          aria-label="WareSync — обзор закупок"
          onClick={(e) => {
            e.preventDefault();
            navigate('dashboard');
          }}
        >
          <span className="brand-icon">
            <img src={new URL('./assets/waresync.png', import.meta.url).href} alt="" />
          </span>
        ...
        <div className="sidebar-bottom">
          ...
          <button className="profile" onClick={openProfile} aria-label="Открыть профиль">
            <span>{initials(user.name)}</span>
            <div>
              <strong>{user.name}</strong>
              <small>{user.job_title || 'Мой профиль'}</small>
            </div>
          </button>
        </div>
      </aside>
      ...
      <div className="main-shell">
        <header className="topbar">
          ...
          <div className="topbar-right">
            ...
            <button className="mini-avatar" onClick={openProfile} aria-label="Мой профиль" title={user.name}>
              {initials(user.name)}
            </button>
          </div>
        </header>
        <main>
              ...
              <div className="scope-bar">
                <div className="scope-input">
                  ...
                  <select
                    aria-label="Склад"
                    value={warehouse}
                    disabled={!!actionBusy || !dataset}
                    onChange={(e) => changeScope(e.target.value, category)}
                  >
                ...
                <div className="scope-input">
                  <select
                    ...
                    value={category}
                    disabled={!!actionBusy || !dataset}
                    onChange={(e) => changeScope(warehouse, e.target.value)}
                  >
          ...
          {page === 'quality' && !run && !busy && <Empty>Для оценки качества запустите расчёт.</Empty>}

          {page === 'data' && scopedDataset && (
            <>
              <section className="data-hero panel">
                ...
                <div>
                  <span className="eyebrow">
                    {scopedDataset.synthetic ? 'ВОСПРОИЗВОДИМОЕ ДЕМО · SEED 42' : 'ЗАГРУЖЕННЫЙ НАБОР'}
                  </span>
                  <h2>{scopedDataset.name}</h2>
                  <p>
                    {dateLabel(scopedDataset.history_start)} — {dateLabel(scopedDataset.as_of)} ·{' '}
                    {num(scopedDataset.sales_count)} продаж · {scopedDataset.products_count} SKU
                  </p>
                  <p>{scopeLabel}</p>
                </div>
              ...
              <div className="source-grid">
                {Object.entries(scopedDataset.sources).map(([key, count]) => (
          ...
          {page === 'history' && (
            <section className="panel">
              <div className="panel-heading">
                ...
                <span className="muted">{scopeLabel} · последние 30</span>
              </div>
              {history.length ? (
                <div className="table-scroll">
                  <table className="order-table">
                      ...
                      {history.map((h) => (
                        ...
                        >
                          <td>
                            ...
                            <small>
                              {h.options.warehouse || 'Все склады'} ·{' '}
                              {dataset?.categories.find((c) => c.id === h.options.category_id)?.name ||
                                'Все категории'}
                            </small>
                          </td>
                          <td>
                ...
                <Empty>
                  {scopeBusy
                    ? 'Загружаем историю…'
                    : 'Для выбранного склада и категории сохранённых расчётов нет.'}
                </Empty>
              )}
            </section>
          )}
          <footer className="page-footer">
            <span>
              WARESYNC <span>×</span> ЭЛЕКТРОКОМПЛЕКТ
            </span>
      ...
      {help && (
        <Modal title="Как работает WareSync" close={() => setHelp(false)}>
